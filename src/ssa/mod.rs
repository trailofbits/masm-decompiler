...
impl From<miden_assembly_syntax::ast::ImmFelt> for Constant {
    fn from(imm: miden_assembly_syntax::ast::ImmFelt) -> Self {
        match imm {
            ...
            miden_assembly_syntax::ast::Immediate::Constant(id) => Constant::Defined(id.to_string()),
        }
    }
}
...
impl From<miden_assembly_syntax::parser::PushValue> for Constant {
    fn from(val: miden_assembly_syntax::parser::PushValue) -> Self {
        match val {
            ...
            miden_assembly_syntax::parser::PushValue::Word(w) => {
                Constant::Word([w.0[0].as_int(), w.0[1].as_int(), w.0[2].as_int(), w.0[3].as_int()])
            }
        }
    }
}
...
pub enum Stmt {
    ...
    Switch { expr: Expr, cases: Vec<(Constant, Vec<Stmt>)>, default: Vec<Stmt> },
