...
pub fn refine(code: &mut Vec<Stmt>) {
    let mut i = 0;
    while i + 1 < code.len() {
        if let Some(Stmt::If { cond, then_body, else_body }) = code.get(i).cloned() {
            // Merge chained identical conditions
            let mut j = i + 1;
            let mut merged = false;
            while j < code.len() {
                if let Some(Stmt::If { cond: other, then_body: t2, else_body: e2 }) =
                    code.get(j).cloned()
                {
                    if other == cond {
                        ...
                        code.remove(j);
                        code[i] =
                            Stmt::If { cond: cond.clone(), then_body: combined, else_body: else_body.clone() };
                        merged = true;
                        break;
                        ...
                        code.remove(j);
                        code[i] = Stmt::If { cond: cond.clone(), then_body: then_block, else_body: else_block };
                        merged = true;
                        break;
                    }
                }
...
fn is_side_effect_free(stmt: &Stmt) -> bool {
    match stmt {
        Stmt::Assign { .. } | Stmt::StackOp(_) | Stmt::Instr(_) | Stmt::Unknown => false,
        Stmt::Expr(_) | Stmt::Branch(_) | Stmt::Phi { .. } | Stmt::Nop | Stmt::Break | Stmt::Continue => true,
        ...
        Stmt::Switch { expr, cases, default } => {
            is_cond_side_effect_free(expr)
                && cases.iter().all(|(_, body)| body.iter().all(is_side_effect_free))
